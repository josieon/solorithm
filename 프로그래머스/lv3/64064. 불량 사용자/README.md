...
### 고찰😊
&nbsp;처음에는 규칙을 찾아 풀려고 했다. HashMap<String, ArrayList>를 이용해 banned_id마다 표현식에 해당하는 user_id를 멤버로 갖는 리스트를 map에 저장했다. 그리고 중복이 없다면 정답은 각 리스트의 사이즈들의 곱이다. 그러나, 중복이 늘 주어졌고, 이를 처리해야했다. banned_id의 표현식이 동일한 경우와, 다른 list의 멤버가 동일하게 선택되는 경우를 고려해 처리해주면 이 문제를 푸는 거라고 생각했으나, 결국엔 못하였다.
&nbsp;이 문제의 데이터 크기는 8이하였고, 사실 이를 보면 브루트포스가 허용되기에 그냥 브루트포스를 이용해 풀면 되는 문제이다. 데이터 크기를 잘 확인하자.💪
